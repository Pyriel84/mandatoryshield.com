...
> ⚠️ **MAIS** : ces en-têtes sont bien **écrits** dans le fichier... et pourtant **non servis** sur les pages HTML. Le scan securityheaders.com renvoie un **`F`**. Voir le bug #0 ci-dessous — c'est LE problème prioritaire.

---

## 🔴 BUG #0 (PRIORITÉ ABSOLUE) : note "F" — en-têtes de sécurité non servis

**Symptôme :** securityheaders.com (31 mai 2026) donne un **`F`**. Tous les en-têtes sont signalés manquants : HSTS, CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy.

**Cause racine — le piège du `add_header` de nginx :**

> Dès qu'un bloc `location` contient **au moins un** `add_header`, il **n'hérite plus** des `add_header` définis dans le bloc parent `server`.

Les 6 en-têtes de sécurité sont dans le bloc `server`. Mais le bloc `location ~* \.html$` possède ses propres `add_header` (Cache-Control + Pragma) → il **n'hérite donc pas** des en-têtes de sécurité. Comme la page `/` sert `index.html` (qui matche ce bloc), tous les en-têtes de sécurité sont **supprimés** sur la réponse HTML.

**Preuve :** dans les *Raw Headers* du scan, on voit `cache-control: no-store...` et `pragma: no-cache` (qui viennent justement de ce bloc `.html`) → la config est bien déployée, mais le bloc écrase l'héritage.

### ✔️ Correctif : répéter les en-têtes dans le bloc `.html`

```nginx
# ── HTML : jamais mis en cache + en-têtes de sécurité répétés ──
location ~* \.html$ {
    add_header Cache-Control "no-store, no-cache, must-revalidate";
    add_header Pragma        "no-cache";

    # OBLIGATOIRE : non hérités car ce bloc a ses propres add_header
...
Note : le `'unsafe-inline'` dans `style-src` règle **en même temps** le bug des styles inline (voir bug suivant).
...
### 💡 Solution durable (recommandée) : factoriser les en-têtes

Pour ne plus jamais oublier un bloc, créer `/etc/nginx/snippets/security-headers.conf` :
...
Puis `include snippets/security-headers.conf;` dans **chaque** bloc `location` (`/`, `.html`, assets).

➡️ Résultat attendu après correctif : **F → A/A+**.
...
- [ ] 🔴🔴 **BUG #0 — Répéter les en-têtes de sécurité dans le bloc `location ~* \.html$`** (cause du `F`) — PRIORITÉ ABSOLUE
- [ ] 🔴 Corriger la CSP vs styles inline (réglé en même temps via `'unsafe-inline'`, ou Option B propre)
- [ ] 🟡 Factoriser les en-têtes dans `snippets/security-headers.conf` + `include` partout
...
- [ ] ⚪ Re-tester sur **securityheaders.com** (viser A/A+) et **ssllabs.com**
- [x] ✅ H1, viewport, canonical, OG, schema → déjà OK
- [x] ✅ En-têtes de sécurité **écrits** dans nginx.conf → OK (mais à servir correctement, cf. bug #0)

---

## 🧠 À retenir (note pédagogique sysadmin)

Le `F` ne venait PAS d'une config manquante, mais de la **règle d'héritage `add_header` de nginx** :
> Un bloc `location` qui définit son propre `add_header` n'hérite plus d'AUCUN `add_header` du bloc `server` parent.

C'est l'une des erreurs de configuration de sécurité les plus fréquentes sous nginx. La parade : factoriser via un `snippet` inclus dans chaque `location`.

---

**Dernière mise à jour:** 31 mai 2026 (audit live + GitHub + scan securityheaders.com = F)  
**Responsable:** Mandatory Shield Company  
**Statut:** 🔴 1 correctif critique à appliquer (bug #0) → passage attendu **F → A/A+**
